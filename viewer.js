...
window.onload = async () => {
  ...
  accessToken = getQueryParam('token');
  ...
  if(userEmail && accessToken){
    ...
    initAuth(); // fallback for browser
  }
};

// ==========================
// GOOGLE AUTH (browser fallback)
// ==========================
let tokenClient;
function initAuth(){
  ...
  tokenClient = google.accounts.oauth2.initTokenClient({
    ...
    callback: resp => {
      ...
      fetchUserEmail().then(checkAccessAndLoad);
    }
  });
...
function renderPdfDoc(pdf){
  ...
  for(let i=1;i<=pdf.numPages;i++) pdf.getPage(i).then(page=> renderPage(page,i));
}
...
function drawWatermark(ctx,width,height){
  ...
  for(let y=0;y<height;y+=stepY){
    for(let x=0;x<width;x+=stepX){
      ctx.save(); ctx.translate(x+stepX/2,y+stepY/2); ctx.rotate(-0.35);
